...
class SharedR:
    ...
    def set_r(self, r):
        assert r in self.r_values, "The rank must be one of the predefined r_values."
        self.current_r = r
...
class ElasticLoRALinear(torch.nn.Linear):
    ...
    def active_sub_adapter(self):
        if self.is_group_head and self.training:
            self.shared_r.randomize_r()
    ...
    @property
    def bias(self):
        if self._bias is not None and self.is_lora_A:
...
def make_lora_elastic(model, r_values, target_modules, share_rank_within_layer=True, config_save_dir=None):
    ...
    def replace_module(layer, named_modules, module_name, shared_r, is_lora_A, is_group_head=False):
            ...
            elastic_lora_linear = elastic_lora_linear.to(module.weight.device)
            parent_module = layer
